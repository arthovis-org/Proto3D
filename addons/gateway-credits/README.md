...
`flowLayout(nodes, connections, opts) → Map(uid → [x, z])` is a pure function: the main chain runs **left → right by topological rank** with equal column gaps (a column is as wide as its widest item, including a row of slot children); parallel branches stack **top → down** inside a column ordered by their predecessors and centred on the chain; **slot children** (anything cabled into `model` / `memory` / `tool…`) leave the ranking and hang in a centred horizontal row **one z-step below their parent** (children of children recurse); `gw-budget` / `gw-meter` sit in a **top row** left-aligned with the first column; loose blocks land in a trailing column; the result has no overlapping footprints (`overlapping()`). `host.layout.graph()` hands the live graph (measured footprints) to it and `host.layout.apply()` moves the blocks as **one undoable `cmd.transform`** through the core history, then frames the view. The sample is built from the same function (`samplePositions()`), and Run sample re-applies it. Slot cables carry the core's `data` type hue, so they already read differently from the event chain; dashed drop-lines from the parent's bottom edge need a core seam (proposal (e) in `addons/README.md`) — until then the agent face shows the three slot chips lit when connected.
