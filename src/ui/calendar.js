// ui/calendar.js — the Home page's Calendar view (ui/home.js mounts it): Month · Week · Agenda
...
// period, T goes to today (while Home shows the calendar).
import { icons } from '../icons.js';
import { isoDate, addDays, daysUntil, fmtDate, fmtHours, PRIORITY_COLOURS } from '../pm/model.js';
import { gridDays, weekStart, monthStart } from '../components/project/calendar.js';
...
export class CalendarView {
  ...
  mount(body) {
    ...
    body.innerHTML = `<div class="home-tools cal-tools">
        ...
        <div class="cal-nav"><button type="button" data-cal="today">Today</button><button type="button" data-cal="prev" aria-label="Previous">${icons.chevron}</button><button type="button" data-cal="next" aria-label="Next">${icons.chevron}</button><h2 class="cal-title" aria-live="polite"></h2></div>
    ...
    body.addEventListener('change', (e) => { if (e.target.dataset.cal === 'time') { this.showTime = e.target.checked; this.render(); } });
  ...
  render() {
    ...
    root.innerHTML = this.mode === 'week' ? this._week(G, items, time) : this._month(G, items, time);
  }
  _chip(it, { cls = '' } = {}) {
  ...
  _click(e) {
    ...
    const mode = t.closest('[data-mode]'); if (mode) { this.mode = mode.dataset.mode; this.offset = 0; this.render(); return; }
  ...
  _move(e) {
    ...
    for (const c of this.body.querySelectorAll('.cal-cell.drop')) c.classList.remove('drop');
