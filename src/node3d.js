...
// out-ports on the right face, a title label and a footer line. Also exports the shared
// port factory so devices use the exact same port anatomy.
import * as THREE from 'three';
import { RoundedBoxGeometry } from 'three/addons/geometries/RoundedBoxGeometry.js';
import {
  palette, categories, portTypes, states, sizes, materials, makeLabel, makeShadowBlob,
} from './theme.js';
...
export function createPort(owner, { name, type = 'signal', dir = 'in' }) {
  ...
  const port = {
    id: nextId++, owner, name, type, dir, group, mesh: ball,
...
export class Node3D extends THREE.Group {
   ...
   * @param {string} [o.subtitle]  footer line (e.g. a type hint or id)
  ...
  constructor(o = {}) {
    ...
    this.titleLabel = makeLabel(this.title, {
      size: sizes.label.title, color: palette.textOnHeader, weight: 600, maxWidth: w - 0.6,
    });
    this.titleLabel.position.set(0, this.header.position.y, d / 2 + 0.04);
    this.add(this.titleLabel);

    // Footer line (dim, small)
    if (o.subtitle) {
      this.footerLabel = makeLabel(o.subtitle, {
        size: sizes.label.small, color: palette.textDim, weight: 500, maxWidth: w - 0.6,
      });
      this.footerLabel.position.set(0, -h / 2 + n.footer / 2, d / 2 + 0.01);
      this.add(this.footerLabel);
    }
  ...
  _addPort(spec, x, y) {
    ...
    const label = makeLabel(spec.name, { size: sizes.label.port, color: palette.textDim, weight: 500 });
  ...
  applyVisual() {
    ...
    if (this.footerLabel) this.footerLabel.material.opacity = disabled ? 0.4 : 1;
  ...
  update(time) {
    ...
    const fade = THREE.MathUtils.clamp(1 - this.position.y / 12, 0.25, 1);
